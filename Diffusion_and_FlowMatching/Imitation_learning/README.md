#### Note
This code example utilizes the frame work from:

https://github.com/berkeleydeeprlcourse/homework_spring2026/tree/main

I use it for my own study purpose. The instruction below is from the original source.
...
To visualize training curves from your experiment logs, use the `plot_training_curves.py` script:

```bash
uv run python src/hw1_imitation/plot_training_curves.py --log-path <path_to_log.csv> --save <output_filename.png>
```

Example:

```bash
uv run python src/hw1_imitation/plot_training_curves.py --log-path exp/flow/seed_42_20260330_110658/log.csv --save training_curves.png
```

This will generate a plot of the training metrics and save it to the specified file.