...
def test_cart_pole_dynamics():
    # Test the dynamics function with a simple case
    init_state = np.array([0.0, 0.3, 0.0, 0.0])  # x, theta, dx, dtheta
    u = 0.0  # no control input

    derivatives = cart_pole.dynamics(init_state, u)
    print("Derivatives at initial state:", derivatives)

    # Simulate for a short time with zero control input
    t_traj = np.linspace(0, 1, 10)  # simulate for 1 second
    u_traj = np.zeros_like(t_traj)  # zero control input

    t, states = cart_pole.simulate(init_state, u_traj, t_traj, isRender=True)
